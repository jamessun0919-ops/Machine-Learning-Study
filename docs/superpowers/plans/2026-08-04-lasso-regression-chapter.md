...
- **Do not shrink `maxIter` below the values specified in Task 1** without checking with the developer first. During design, a `maxIter=300000` combined with a too-small effective iteration budget silently produced wrong zero-counts (the algorithm exited before truly converging) — this looked like a working implementation (no errors, plausible-looking numbers) but was numerically wrong. Independent re-verification during the final review confirmed **`maxIter` is the sole binding constraint here** — a tol sweep from 1e-6 to 1e-12 had no effect on the outcome once `maxIter` was large enough (≈56,000+); the exact same wrong-answer bug reproduces at the shipped `tol=1e-12` if `maxIter` is set below ~56,000. Do not "safely" lower `maxIter` as a performance tweak while leaving `tol` untouched — that reintroduces this exact bug. `maxIter=200000, tol=1e-12` was verified sufficient for the whole λ whitelist (worst case converged in 56,056 iterations) and is fast (~227ms for all 5 λ values combined).
