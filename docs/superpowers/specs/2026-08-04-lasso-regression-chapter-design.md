...
- **`fitLassoRegression(features, target, lambda)` 求解器**：coordinate descent + soft-thresholding，懲罰項跳過截距（index 0），於 `src/lib/regression.ts` 新增，重用既有 `rmse`/`rSquared`/`predict`；生產參數 `maxIter=200000, tol=1e-12`（已驗證此參數組合在最終白名單五個值皆能真正收斂到正確解，不會因提前跳出而誤判歸零係數數量——設計階段曾誤用較寬鬆的 `tol=1e-10`／較小的 `maxIter=300000` 組合，結果讓 λ=0.01 附近的係數被誤判為未歸零，已排除此陷阱）。
